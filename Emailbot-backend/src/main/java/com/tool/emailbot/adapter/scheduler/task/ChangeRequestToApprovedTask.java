...
/**
 * The aim of the service is to auto approve an email request for the Use Case #4.
 *
 * @author Jovani Rico (jovanimtzrico@gmail.com)
 */
public class ChangeRequestToApprovedTask extends AssertionConcern implements Task {
    ...
    @Inject
    public ChangeRequestToApprovedTask(PeticionRepository repository,
    ...
    public void task() {
        List<Peticion> peticiones = repository.find(Estatus.NOTIFICADO, fetchSize);
    ...
    public void processRequest(Peticion peticion) {
        try {
            peticion.transitionFromNotificado();
