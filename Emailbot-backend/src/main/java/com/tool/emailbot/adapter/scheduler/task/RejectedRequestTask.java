 ...
 */
public class RejectedRequestTask extends AssertionConcern implements Task{
    ...
    @Inject
    public RejectedRequestTask(PeticionRepository repository,
    ...
    public void task() {
        List<Peticion> peticiones = repository.find(Estatus.RECHAZADA, fetchSize);
    ...
    public void processRequest(Peticion peticion) {
        try {
	    notificationService.sendEmail(peticion.getTrabajador().getPersona().getInformacionContacto().getEmail());
            peticion.transitionFromExitosa();
